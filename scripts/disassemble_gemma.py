...
Combines every channel into one listing. For ALL 208 heads (26L x 8H): the ADDRESSING profile (attention-
bucket split self/sink/prev/structural/local/long_range -> dominant mode) and IDIOM tags (prev-token /
duplicate / induction, z>1.5). For the Gemma Scope SAE layer (12): the CONTENT opcode (QK B_h top binding +
legibility, GQA + RMSNorm-fold + unrotated content-QK) and the OV WRITE (V_h = D @ (W_O^h W_V^{kv}) @ D.T:
source-feature -> written-feature), over diverse content operands (Gemma Scope decoder dirs). Reads
gemma_causal_summary.json if present to annotate which idiom heads are causally load-bearing.

Writes a human-readable listing to runs/gemma2_disassembly.txt + a JSON. Runs on cuda (bf16). The headline:
the GPT-2 disassembly framework, ported whole to a RoPE/GQA/RMSNorm model.
"""
...
def main(argv=None):
    ...
    p.add_argument("--causal", type=Path, default=Path("runs/gemma_causal_summary.json"))
    p.add_argument("--output", type=Path, default=Path("runs/gemma2_disassembly.json"))
    p.add_argument("--txt", type=Path, default=Path("runs/gemma2_disassembly.txt"))
    ...
    # ---- pass 1: behavioral + coverage (all heads) + resid@SL for the SAE ----
    ...
    for L in range(nL):
        for h in range(H):
            ...
            rec = {"head": f"{L}.{h}", "dominant": BUCKETS[int(np.argmax(f))],
                   ...
                   "causal_load_bearing": causal.get(f"{L}.{h}", [])}
            if L == SL:
    ...
    out = {"experiment": "Gemma-2-2B disassembly", "model": args.model, "n_layers": nL, "n_heads": H,
           "sae_layer": SL, "n_operands": nt, "attention_budget": budget, "heads": rows}
    args.output.parent.mkdir(parents=True, exist_ok=True)
    args.output.write_text(json.dumps(out, indent=2, default=float))

    # ---- human-readable listing ----
    lines = [f"GEMMA-2-2B DISASSEMBLY  ({nL} layers x {H} heads; GQA n_kv={n_kv}, RoPE, RMSNorm)",
             f"corpus={args.corpus}  tokens={P}  SAE-layer={SL} (Gemma Scope, {nt} content operands)", ""]
    ...
    for L in range(nL):
        ...
        for h in range(H):
            ...
            lines.append(f"  {r['head']:>5}  addr={r['dominant']:<11}{tag}{cz}{extra}")
    args.txt.write_text("\n".join(lines) + "\n")
    print("\n".join(lines[:6]))
    n_idiom = sum(1 for r in rows if r["idioms"]); n_leg = sum(1 for r in rows if r.get("sae_opcode", {}).get("qk_z", 0) > 2)
    print(f"\n[summary] {n_idiom}/{len(rows)} heads carry an idiom tag; layer-{SL} content opcodes legible (z>2): {n_leg}/{H}")
