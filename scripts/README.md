...
| `circuit_atlas.py` | **the circuit ATLAS — composed multi-operator circuits surveyed/collected across models.** Cross-model circuit-EDGE liveness (path-patch: remove the writer from the reader's key → collapse %) for induction / positional-broadcast / duplicate × 6 models, **+ harvests** the GPT-2 discovery artifacts (`composition_dag`/`vcomposition`/`ioi_causal`/`validate_new_edges`) into circuit records: IOI Q-chain (5 live, self-repair), V-virtual-heads (5.9→6.7), 22 novel-live discovered edges. Finding: **induction edge live in all 6 (stronger in RoPE +70/+89%); positional-broadcast GPT-2-small/medium-only.** → `runs/disassembly/circuits/atlas_summary.json` (see [`docs/circuits/`](../docs/circuits/)). |
