...
| `self_repair.py` | **IOI self-repair, made causal** — why name-movers read ~0 under mean-ablation (#33's caveat): ablating primary name-movers drops IOI logit-diff −0.002, but primaries+backups together drops it −1.04; backups idle with primaries present (Δ+0.26), carry the circuit once primaries go (Δ+1.04, 4×). The named circuit ships hot spares (see [`docs/DECOMPILATION.md`](../docs/DECOMPILATION.md)). |
| `residual_vm_debugger.py` | **a PROGRAMMATIC discovery engine over the forward pass** (automation-first, UI-optional) — the tool used to *find more operators/circuits*, not a human REPL. API: `trace` (per-head/MLP writes), `intervene` (ablate/preserve → next-token KL), `logit_lens_step` (where the answer is decided), **`attribution_sweep`** (ablate every head+MLP, rank by causal effect, flag UNNAMED strong components = candidate new ops), **`edge_probe`** (upstream writers → candidate circuit edges). Run on GPT-2 it surfaces **MLP0 as the top component for every behaviour** (induction/IOI/generic) and discovered unnamed candidates (induction→7.6, IOI→5.9/8.3) → feeds the operator/circuit catalog. → `runs/disassembly/residual_vm_debugger_summary.json`. |
| `circuit_atlas.py` | **the circuit CATALOG (survey matrix) — composed multi-operator circuits surveyed/collected across models.** Cross-model circuit-EDGE liveness (path-patch: remove the writer from the reader's key → collapse %) for induction / positional-broadcast / duplicate × 6 models, **+ harvests** the GPT-2 discovery artifacts (`composition_dag`/`vcomposition`/`ioi_causal`/`validate_new_edges`) into circuit records: IOI Q-chain (5 live, self-repair), V-virtual-heads (5.9→6.7), 22 novel-live discovered edges. Finding: **induction edge live in all 6 (stronger in RoPE +70/+89%); positional-broadcast GPT-2-small/medium-only.** → `runs/disassembly/circuits/atlas_summary.json` (see [`docs/circuits/`](../docs/circuits/)). |
| `circuit_catalog_doc.py` | **generates the circuit-catalog docs** — `docs/circuits/README.md` (cross-model edge matrix + circuit inventory + taxonomy circuit→edge→op-node) + one `docs/circuits/<circuit>.md` page per circuit. Mirror of `operator_catalog_doc.py`. |
| `operator_atlas.py` | **the operator CATALOG (survey matrix) — every (behavioural) operator class × every model, one master matrix.** 7 universal/addressing classes (prevtok/induction/duplicate/sink/self/local/structural) × 6 models (GPT-2 small/medium/large + Gemma-2-2B/Llama-3.2-1B/Qwen-2.5-1.5B), arch-generic. Per cell: behavioural **signal**, **membership** (#heads), top head+depth, uniform **causal ΔNLL**. Findings: induction universal (.91–.99); **sink present in 5/6 but ABSENT in Gemma** (and not prose-load-bearing anywhere = present≠depended-on); RoPE leans on `self` (Qwen causal +3.77). IOI circuit classes are GPT-2-only (dossiers). → `runs/disassembly/operators/atlas_summary.json` (see [`docs/operators/`](../docs/operators/)). |
| `operator_catalog_doc.py` | **generates the catalog docs from the JSON artifacts** — `docs/operators/README.md` (master signal/membership/causal matrices + taxonomy: class→instance→variant) + one `docs/operators/<op>.md` page per operator class (catalog row + the deep GPT-2 dossier). Re-run after new survey/dossier runs to keep the catalog in sync. |
| `operator_dossier.py` | **a DEEP per-operator dossier — the full battery on ONE instruction, not one measurement scattered per script.** `--op induction` (default; also `prevtok\|duplicate\|name_mover\|s_inhibition`) runs six sections: **A** behavioural head-ID, **B** causal×5-tasks, **C** K/V channel decomposition (match vs move), **D** composition in/out edges (local call-graph), **E** the redundancy/ablation curve, **F** cross-model signal. Induction on GPT-2: heads 5.1/5.5/6.9/7.10/7.2; KEY carried by prev-token 4.11 (+43%, 90.8× concentration) / VALUE distributed; OUT→value feeds L6/7 "virtual heads"; redundancy **superadditive** (5h +6.39 ≫ best single +1.70); signal universal (Qwen 1.00). Surfaces a head-set sensitivity the piecemeal scripts hid (successor served by the lit set, not the behavioural top-5). Consolidates `composition_dag`/`circuit_content_patch`/`self_repair`/`instruction_reuse` (see [`docs/DECOMPILATION.md`](../docs/DECOMPILATION.md)). |
...
| `circuit_content_patch.py` | **generalizes the patch across circuits & channels (match vs move)** — key-only (match) + value-only (move) patch on prev-token (positional) vs induction + duplicate (content), 4 models. **KEY/match:** key-content dependence is about ADDRESSING TYPE not architecture — positional (prev-token) is GPT-2-only (+22% vs RoPE +0%), content (induction) universal (+17…+89% all 4). **VALUE/move:** universal everywhere (values are never rotated — even prev-token's value is content-dependent in RoPE where its key isn't), and more distributed than the sparse key channel. So only positional *matching* is architecture-specific (see [`docs/DECOMPILATION.md`](../docs/DECOMPILATION.md)). |
...
| `ssm_induction.py` | **does the in-context-copy op survive a non-attention mixer?** — exhaustive sweep: Mamba 130m/370m/790m (pure SSM, no heads/QK) vs GPT-2 small/medium/large, 3 seeds × 3 lengths. Reports raw data, holds the verdict: induction GAIN (1st−2nd copy NLL) is large and band-tight (+12.1..+12.5) in **both** families — the copy *capability* is mixer-class-invariant; localization differs (Mamba concentrates on an early layer + mid-deep peak, GPT-2 distributes) but whole-layer ablation is coarse (gpt2-large's L0 spike is a detokenizer artifact). Does NOT claim the *mechanism* is the same — no head-resolution in an SSM (see [`docs/DECOMPILATION.md`](../docs/DECOMPILATION.md)). |
