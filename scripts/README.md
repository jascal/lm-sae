...
| `dag_recompile.py` | **M1↔M2 bridge** — feeds the M2 live sub-DAG into M1's reconstruction-coverage harness: the path-patch-confirmed induction+IOI sub-DAG is a coverage-efficient keep-set (matches/beats equal-size greedy top-importance, ≫ random) — but raw ΔTV-connectivity is *not* a generic output-importance proxy (ΔTV reshapes attention ≠ KL importance). Consumes `composition_dag_summary.json` (see [`docs/DECOMPILATION.md`](../docs/DECOMPILATION.md)). |
...
| `cross_model_ceiling.py` | **decompilation milestone 5** — arch-generic reconstruction-coverage ceiling (M1's metric) on GPT-2 + Gemma-2/Llama-3/Qwen-2.5: the named induction circuit beats random in every model (mechanisms invariant) but its coverage-share is **far higher for GPT-2** (20% / 4.1×) than the larger RoPE models (3–9% / 1.4–2.9×) → the decompilable *fraction* is **not** architecture-invariant (bigger models distribute the computation more). Op-selection ceiling; forge-basis stays SAE/GPU-gated (see [`docs/DECOMPILATION.md`](../docs/DECOMPILATION.md)). |
