...
| `circuit_content_patch.py` | **generalizes the key-only patch across circuits** — same faithful key-patch on prev-token (positional) vs induction + duplicate (content), 4 models. Finds key-content dependence is about ADDRESSING TYPE not architecture: positional (prev-token) is GPT-2-only (+22% vs RoPE +0%), but content (induction) is universal (+17…+89% in all 4) — RoPE moves only the *positional* register to the rotation; token-identity matching always lives in the key (see [`docs/DECOMPILATION.md`](../docs/DECOMPILATION.md)). |
