...
def main(argv=None):
    ...
    p.add_argument("--opcodes", type=Path, default=Path("runs/disassembly/qk_opcode_table_summary.json"),
                   help="token-operand opcode table (content-legibility credit)")
    p.add_argument("--sae-opcodes", type=Path, default=Path("runs/disassembly/sae_opcode_table_summary.json"),
                   help="SAE-feature opcode table (additional content credit where it ran)")
    p.add_argument("--output", type=Path, default=Path("runs/disassembly/coverage_scorecard_summary.json"))
