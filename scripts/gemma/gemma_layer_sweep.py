...
def main(argv=None):
    ...
    for L in layers:
        ...
        b = best[1]; _mz = float(np.nanmean(zs)); _mzs = f"{_mz:+.2f}" if _mz == _mz else "n/a"
        print(f"  layer {L:>2}: ops {nt:>2} cells {int(supp.sum()):>4}  legible(z>2) {nlz}/{H}  "
              f"mean_z {_mzs}  best {b[0]+': '+repr(b[1])+'->'+repr(b[2]) if b else 'n/a'}")
    ...
    for r in rows:
        ...
        mz = f"{r['mean_z']:+.2f}" if r["mean_z"] == r["mean_z"] else "n/a"  # NaN-safe: starved layer (too few operands)
        lines.append(f"{r['layer']:>6} {r['n_operands']:>4} {r['supported_cells']:>6} "
                     f"{str(r['n_legible_z2'])+'/'+str(H):>13} {mz:>7}  {bs}")
